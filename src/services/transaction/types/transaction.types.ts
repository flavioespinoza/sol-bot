...
export const TransactionConfigMap: Record<TransactionType, TransactionConfig> = {
  ...
  [TransactionType.WITHDRAW]: {
    label: ({ amount, token } = {}) =>
      amount && token ? `Withdraw ${amount} ${token}` : "Withdraw",
  },
  ...
  [TransactionType.DEPOSIT_STAKE]: {
    label: ({ amount, token } = {}) =>
      amount && token
        ? `Authorize stake account and deposit ${amount} ${token}`
  ...
  [TransactionType.WITHDRAW_ALL]: {
    label: ({ amount, token } = {}) =>
      amount && token ? `Withdraw ${amount} ${token}` : "Withdraw all",
  },
};
